...
//
// Why: Google deprecated the legacy @mediapipe/hands JS solution in
// favor of MediaPipe Tasks. The new HandLandmarker:
//   - Uses a modern WASM+GPU delegate pipeline (explicitly requestable
//     via `delegate: 'GPU'`), generally faster and better optimized on
//     mobile than the legacy solution's fixed pipeline.
//   - Ships a single unified API surface (Tasks) that MediaPipe
//     continues to develop; the legacy package receives no further
//     updates.
//   - Lets us pick model asset variants explicitly via
//     modelAssetPath, same idea as the old modelComplexity: 0/1 knob.
//
// Everything else — pinch detection, XR camera-source integration,
// smoothing, laser/dot rendering, throttling — is UNCHANGED from the
// previous version. Only the underlying hand-detection engine and how
// we call it (detectForVideo() on an ImageBitmap/canvas instead of
// hands.send()) is different.
//
...
// same spirit as the old modelComplexity: 0. Hosted on Google's
// public MediaPipe model CDN.
